...
{
    // Cari data berdasarkan UID
    $member = $this->memberModel->where('uid', $uid)->first();

    // Jika tidak ditemukan, redirect dengan pesan error
    if (!$member) {
        return redirect()->to('admin/members')
            ->with('msg', 'Anggota tidak ditemukan.')
            ->with('error', true);
    }

    // Hapus data anggota berdasarkan ID (bukan UID)
    $deleteSuccess = $this->memberModel->delete($member['id']);

    if (!$deleteSuccess) {
        return redirect()->back()
            ->with('msg', 'Gagal menghapus anggota.')
            ->with('error', true);
    }

    // Jika ada QR code, hapus file-nya juga
    ...
    return redirect()->to('admin/members')
        ->with('msg', 'Anggota berhasil dihapus.');
}
public function exportPdf()
{
    ...
    $options->set('isRemoteEnabled', true); // untuk load logo dari URL kalau pakai

    $dompdf = new Dompdf($options);

    $html = view('admin/members/pdf_report', ['members' => $members]);

    $dompdf->loadHtml($html);
    $dompdf->setPaper('A4', 'landscape');
    $dompdf->render();

    $dompdf->stream('Laporan_Data_Anggota_' . date('Ymd_His') . '.pdf', ['Attachment' => false]);
}
}
