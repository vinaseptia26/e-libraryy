...
{
    $data['loans'] = $this->loanModel->findAll();
    return view('admin/loans/print', $data); // Buat file ini
}
...
public function exportExcel()
{
    ...
    $sheet->setCellValue('E1', 'Tenggat');

    // Isi baris data
    $row = 2;
    foreach ($loans as $loan) {
        $sheet->setCellValue("A$row", "{$loan['first_name']} {$loan['last_name']}");
        $sheet->setCellValue("B$row", "{$loan['title']} ({$loan['year']})");
        $sheet->setCellValue("C$row", $loan['quantity']);
        $sheet->setCellValue("D$row", date('d/m/Y', strtotime($loan['loan_date'])));
        $sheet->setCellValue("E$row", date('d/m/Y', strtotime($loan['due_date'])));
        $row++;
    }

    // Set auto-width (opsional)
    ...
    // Clear output buffer sebelum mengirim file
...
public function exportPDF()
{
    ...
    $html = view('admin/loans/print', $data); // view HTML yang sama seperti cetak
    $dompdf->loadHtml($html);
    $dompdf->setPaper('A4', 'landscape');
    $dompdf->render();
    $dompdf->stream('laporan_peminjaman.pdf', ['Attachment' => 0]);
}
public function returnLoan($uid)
{
    $loan = $this->loanModel->where('uid', $uid)->first();
    ...
    return redirect()->back()->with('msg', 'Peminjaman berhasil dikembalikan.');
}
}
