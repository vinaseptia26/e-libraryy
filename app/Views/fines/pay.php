      ...
      <div class="col-12 col-md-6">
        ...
        <form action="<?= base_url("admin/fines/{$return['uid']}"); ?>" method="post">
          ...
          <div class="mb-3">
            ...
            <input type="number" class="form-control <?php if ($validation->hasError('nominal')) : ?>is-invalid<?php endif ?>" id="nominal" name="nominal" value="<?= $oldInput['nominal'] ?? ''; ?>" placeholder="sisa: <?= ($return['fine_amount'] ?? 0) - ($return['amount_paid'] ?? 0); ?>" min="5000" max="<?= ($return['fine_amount'] ?? 0) - ($return['amount_paid'] ?? 0); ?>" aria-describedby="nominalHelp" required>
